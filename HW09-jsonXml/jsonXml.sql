...
drop table if exists Warehouse.StockItems1;
DECLARE @xmlDocument XML;
SELECT @xmlDocument = BulkColumn
FROM OPENROWSET(BULK 'C:\Temp\StockItems.xml', SINGLE_CLOB) as t;
DECLARE @docHandle INT;
EXEC sp_xml_preparedocument @docHandle OUTPUT, @xmlDocument;

SELECT SupplierID, StockItemName, UnitPackageID, OuterPackageID, QuantityPerOuter, 
		TypicalWeightPerUnit, LeadTimeDays, IsChillerStock, TaxRate, UnitPrice
	into Warehouse.StockItems1
	...
	WITH ( 
		...
		group by SupplierID, StockItemName, UnitPackageID, OuterPackageID, QuantityPerOuter, 
		TypicalWeightPerUnit, LeadTimeDays, IsChillerStock, TaxRate, UnitPrice

;merge Warehouse.StockItems as target
using Warehouse.StockItems1 as source on source.StockItemName = target.StockItemName
when matched then update set target.StockItemName = source.StockItemName
when not matched by target then insert 
(SupplierID, StockItemName, UnitPackageID, OuterPackageID, QuantityPerOuter, 
		TypicalWeightPerUnit, LeadTimeDays, IsChillerStock, TaxRate, UnitPrice, LastEditedBy) 
values(
    source.SupplierID, source.StockItemName, source.UnitPackageID, 
...
SET @x = (
		SELECT *
		FROM OPENROWSET(BULK 'C:\Temp\StockItems.xml', SINGLE_BLOB) AS d
		);
...
SELECT  
  ...
  TypicalWeightPerUnit = t.Item.value('(Package/TypicalWeightPerUnit)[1]', 'decimal'), 
  LeadTimeDays = t.Item.value('(LeadTimeDays)[1]', 'int'), 
  IsChillerStock = t.Item.value('(IsChillerStock)[1]', 'int'),
  TaxRate = t.Item.value('(TaxRate)[1]', 'decimal'), 
  UnitPrice = t.Item.value('(UnitPrice)[1]', 'decimal')
  into Warehouse.StockItems1
